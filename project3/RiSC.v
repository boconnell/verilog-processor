...
module tlb (clk, reset, vpn1, asid1, vpn2, asid2, pfn1, pfn2, miss1, miss2, map2, write);
    ...
    //none of these implemented for me
	assign	tlbA__we = write & ctr_in; //(tlbB_v | ctr_in);
	assign	tlbA_asid__in = asid2;
	assign	tlbA_vpn__in = vpn2;
	assign	tlbA_pfn__in = map2;
	assign	tlbB__we = write & ctr_out; //((tlbA_v | ctr_out) & ~tlbA__we);
	...
	assign	tlbB_pfn__in = map2;
        ...
        wire            asid2_is_kphys = asid2 == 0; //possibly add & ~vpn2[7] to both of these
...
module RiSC (clk, reset);
	...
	assign  IFID_exc__in =          (Pstomp) ? 7'd0 
					...
					: 7'd0;
	assign	IFID_we =		(~Pstall | x);




	assign	TLB_vpn1__in =		PC__out[`BYTE_TOP]; //top 8 bits of PC__out
	assign	TLB_asid1__in =		(kmode) ? 6'd0 : asid; //output of MUXk
	...
	//either IDEX_pc__out or IDEX_op1__out
	...
	assign	EXMEM_stdata__in = 	IDEX_op2__out; //(idex_is_tlbw) ? IDEX_op1__out : 
